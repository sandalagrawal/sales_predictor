...
def forecast_sales(customer, train_end_date="2024-05-01", forecast_start_date="2024-06-01", forecast_end_date="2026-12-01"):
    sales_data = wide_df.loc[customer].T
    sales_data.index = pd.to_datetime(sales_data.index, format="%m/%y", errors="coerce")
    
    train_data = sales_data[:train_end_date]
    if train_data.isna().all():
        return None, None, None  # Handle cases where there's no training data
...
if st.button("Generate Forecast"):
    actual_data, forecast_data, conf_int = forecast_sales(customer_name)
    
    if actual_data is None:
        st.error("No data available for this customer.")
    else:
        # Ensure the actual data includes predictions for Jun-Dec 2024
        forecasted_june_dec_2024 = forecast_data["2024-06-01":"2024-12-01"]
        actual_data = actual_data["2024-06-01":"2024-12-01"]
        combined_actual = pd.concat([actual_data, forecasted_june_dec_2024]).sort_index()
        
        forecast_data = forecast_data["2025-01-01":]
        conf_int = conf_int["2025-01-01":]
    
        # Plot actual and forecasted sales
        plt.figure(figsize=(10, 5))
        plt.plot(combined_actual.index, combined_actual.values, label="Actual & Predicted Sales (Jun-Dec 2024)", marker="o")
        plt.plot(forecast_data["Date"], forecast_data["Predicted Sales"], label="Forecasted Sales (2025-2026)", linestyle="dashed", marker="o", color="red")
